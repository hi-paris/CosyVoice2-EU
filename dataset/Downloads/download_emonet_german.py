...
def check_existing_temp_data(temp_dir):
    ...
    for item in os.listdir(temp_dir):
        ...
        if os.path.isdir(item_path) and item.startswith("de_") and item != "hf_cache":
            # Extract speaker and file_prefix from directory name
            try:
                speaker = item.split("_")[1]
                file_prefix = item.split("de_")[1]
...
def copy_files_batch(args):
    ...
    for utt_info in utt_list:
        ...
        # Create unique filename: speaker_subset_utt.wav with DE suffix
...
def download_process_and_split_emonet_german(data_dir, splits=None, skip_download=False):
    """Download, process, and split German EmoNet dataset in one optimized workflow."""
    if splits is None:
    ...
    # Create directories
    temp_dir = os.path.join(data_dir, "temp_emonet_german_optimized")
    output_root = os.path.join(data_dir, "EmoNet_german_split")
    ...
    if not skip_download or not downloaded_dirs:
        ...
        # Filter German files
        german_files = [f for f in files if f.startswith("de_")]
        logger.info(f"Found {len(german_files)} German files to process")
        
        # Phase 1: Parallel download of tar files
        logger.info("Phase 1: Downloading tar files in parallel...")
        download_args = [(repo_id, file_path, temp_dir) for file_path in german_files]
        
        downloaded_dirs = []
        max_download_workers = min(10, len(german_files))  # Reasonable parallel downloads
        ...
        with ThreadPoolExecutor(max_workers=max_download_workers) as executor:
            ...
            for future in tqdm(as_completed(future_to_file), total=len(download_args), 
                ...
                if extract_dir and os.path.exists(extract_dir):
                    ...
                    file_prefix = file_path.split("de_")[1].split(".tar")[0]
    ...
    # Report results
    logger.info(f"\nGerman EmoNet dataset download and split complete!")
    logger.info(f"Total utterances processed: {len(all_utterances)}")
    ...
    logger.info("✅ German dataset is ready for training!")
...
if __name__ == "__main__":
    ...
    skip_download = False  # Change to False if you want fresh download
    
    total_files = download_process_and_split_emonet_german(data_dir, splits, skip_download)
