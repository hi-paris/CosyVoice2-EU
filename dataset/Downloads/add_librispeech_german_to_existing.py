...
def process_mls_german(output_root, splits):
    ds = load_dataset(
        "facebook/multilingual_librispeech",
        "german",
        cache_dir="/tsi/hi-paris/tts/Luka/hf_cache"
    )
    logger.info("Loaded MLS German dataset: splits = %s", list(ds.keys()))
    ...
    for split in ["train", "dev", "test"]:
        ...
        for item in tqdm(subset, desc=f"Processing MLS-German-{split}"):
            ...
            try:
                ...
                # IMPORTANT: Add libri_ prefix to speaker to distinguish from EmoNet and add DE suffix
                target_dir = Path(output_root) / split / f"libri_{speaker}_DE" / str(chapter)
                target_dir.mkdir(parents=True, exist_ok=True)
                
                final_id = f"libri_{speaker}_DE_{split}_{utt_id}"
    ...
    logger.info("Processed %d MLS German utterances", len(entries))
    return entries
...
def main():
    ...
    logger.info("Adding MLS German data to existing dataset...")
    logger.info(f"Target directory: {root}")
    
    # Only process LibriSpeech German - skip EmoNet since it's already there
    process_mls_german(root, splits)
    
    logger.info("✅ Added LibriSpeech German data to existing combined dataset")
