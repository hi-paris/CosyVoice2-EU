...
utt2text_global = {}
utt2spk_global = {}
utt2embedding_global = {}
spk2embedding_global = {}
utt2speech_token_global = {}
utt2reject_speech_token_global = {}
dpo_global = False

def init_worker(utt2wav, utt2text, utt2spk, utt2embedding, spk2embedding, utt2speech_token, utt2reject_speech_token, dpo):
    """Initialize worker process with shared data"""
    global utt2wav_global, utt2text_global, utt2spk_global, utt2embedding_global, spk2embedding_global, utt2speech_token_global, utt2reject_speech_token_global, dpo_global
    utt2wav_global = utt2wav
    utt2text_global = utt2text
    utt2spk_global = utt2spk
    utt2embedding_global = utt2embedding
    spk2embedding_global = spk2embedding
    utt2speech_token_global = utt2speech_token
    utt2reject_speech_token_global = utt2reject_speech_token
    dpo_global = dpo
...
def job(utt_list, parquet_file, utt2parquet_file, spk2parquet_file):
    try:
        ...
        for utt in tqdm(utt_list):
            data = open(utt2wav_global[utt], 'rb').read()
            data_list.append(data)
        wav_list = [utt2wav_global[utt] for utt in utt_list]
        text_list = [utt2text_global[utt] for utt in utt_list]
        spk_list = [utt2spk_global[utt] for utt in utt_list]
        uttembedding_list = [utt2embedding_global[utt] for utt in utt_list]
        spkembedding_list = [spk2embedding_global[utt2spk_global[utt]] for utt in utt_list]
        speech_token_list = [utt2speech_token_global.get(utt, []) for utt in utt_list]
        if dpo_global:
            reject_speech_token_list = [utt2reject_speech_token_global[utt] for utt in utt_list]
        ...
        print(f'Completed {parquet_file} in {time.time() - start_time:.2f} seconds')
...
if __name__ == "__main__":
    ...
    pool = multiprocessing.Pool(processes=args.num_processes, initializer=init_worker, 
                                initargs=(utt2wav, utt2text, utt2spk, utt2embedding, spk2embedding, 
                                         utt2speech_token, utt2reject_speech_token if args.dpo else None, args.dpo))
